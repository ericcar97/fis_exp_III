...
###Defying the model
def B_model(x,x0,radio,vueltas):
    return mu_0*vueltas*corriente*radio**2/(radio**2+ (x - x0) **2)**(3/2)/2

###Model fitting
x0, radio, vueltas = 0.1, 5.87 , 500
init_values = [ x0, radio , vueltas ]
...
residuals = y_data - B_model(x_data,fit[0],fit[1],fit[2])
...
yfit = B_model(xfit,fit[0],fit[1],fit[2])
