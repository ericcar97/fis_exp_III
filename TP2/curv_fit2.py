...
###Defying the model
def P_model(v,v_0,Q):
    return -arctan(Q*(v/v_0)*(1-(v_0/v)**2))*180/pi

###Model fitting
v_0, Q = 6070. , 0.98
init_values = [v_0,Q]
fit, covariance = curve_fit(P_model,x_data,y_data,
                            p0 = init_values,
                            absolute_sigma = True,
                            sigma = y_error)
nu = N - 2 #### degrees of freedom = number of data points - number of parameters fitted
std_dev = sqrt(diag(covariance))
residuals = y_data - P_model(x_data,fit[0],fit[1])
